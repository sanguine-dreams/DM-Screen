...
export async function getNotes(cId) {
  const response = await db.get(
    `/api/collections/notes/records?filter=(CampaignHead?="${cId}")`
  );
...
export async function getCampaigns(userId) {
  const response = await db.get(
    `/api/collections/campaign/records/?filter=(DungeonMaster?="${userId}")`
  );
  const result = response.data.items;
...
export async function getPlayers(cId) {
  const response = await db.get(
    `/api/collections/players/records/?filter=(CampaignHead?="${cId}")`
  );
  const result = response.data.items;
